(function(){
  ...
  const arrText=v=>Array.isArray(v)?v.join(', '):String(v||'');
  const splitTags=v=>String(v||'').split(/[、,\s/]+/).map(x=>x.trim()).filter(Boolean);
  ...
  function mapEvent(e){
    ...
    return {
      ...
      lat:e.lat||'',lng:e.lng||'',mood_tags:e.mood_tags||[],time_tags:e.time_tags||[],area_tags:e.area_tags||[],budget_max:e.budget_max||'',indoor:e.indoor===true,outdoor:e.outdoor!==false,safety_level:e.safety_level||4,
  ...
  window.submitForm=async function(){
    ...
    const payload={
      ...
      lat:$('#f-lat')?.value?Number($('#f-lat').value):null,lng:$('#f-lng')?.value?Number($('#f-lng').value):null,budget_max:$('#f-budget-max')?.value?Number($('#f-budget-max').value):null,
      mood_tags:splitTags($('#f-mood-tags')?.value),time_tags:splitTags($('#f-time-tags')?.value),area_tags:splitTags(loc+' '+($('#f-addr')?.value||'')),indoor:!!$('#f-indoor')?.checked,outdoor:!!$('#f-outdoor')?.checked,safety_level:Number($('#f-safety')?.value||4),
    ...
    try{
      ...
      if(res.error){
        ...
        if(msg.includes('thumbnail_url')||msg.includes('gacha_enabled')||msg.includes('gacha_conditions')||msg.includes('production_option')||msg.includes('lat')||msg.includes('lng')||msg.includes('budget_max')||msg.includes('mood_tags')||msg.includes('time_tags')||msg.includes('area_tags')||msg.includes('indoor')||msg.includes('outdoor')||msg.includes('safety_level')||msg.includes('char_image_url')||msg.includes('char_name')||msg.includes('char_desc')||msg.includes('status')||msg.includes('submitted_by_role')||msg.includes('submitted_at')||msg.includes('review_note')||msg.includes('rejection_reason')||msg.includes('data_plan')){
          const slim={...payload};
          ...
          delete slim.lat; delete slim.lng; delete slim.budget_max; delete slim.mood_tags; delete slim.time_tags; delete slim.area_tags; delete slim.indoor; delete slim.outdoor; delete slim.safety_level;
