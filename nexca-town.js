(function(){
  ...
  function renderScreen(v){
    ...
    if(v==='chars')s.innerHTML=head('キャラ育成')+`<div class="nxt-grid">${Object.values(state.characters).map(c=>`<div class="nxt-card"><div class="nxt-big">${c.id==='neku'?'🌟':c.id==='furugy'?'🧵':c.id==='caferu'?'☕':'🎪'}</div><h3>${safe(c.name)} Lv${c.level}</h3><p>${safe(c.dialogue)}<br>${safe(c.skillDescription)}</p><div class="nxt-exp"><i style="width:${Math.min(100,c.exp/(70+c.level*25)*100)}%"></i></div><button class="nxt-play" onclick="NexcaTown.train('${c.id}')">育成する</button></div>`).join('')}</div>`;
    else if(v==='games')s.innerHTML=head('ミニゲーム広場')+`<div class="nxt-grid">${D().games.map(g=>`<div class="nxt-card"><div class="nxt-big">${g[0]==='coord'?'👗':g[0]==='order'?'☕':g[0]==='festival'?'🎪':g[0]==='manager'?'📈':'📣'}</div><h3>${g[1]}</h3><p>${g[2]} / 難易度 ${g[3]}<br>関連キャラ: ${g[4]}<br>報酬: ${g[5]}</p><button class="nxt-play" onclick="NexcaTown.startGame('${g[0]}')">プレイ</button></div>`).join('')}</div>`;
  ...
  function gameHtml(type){
    if(type==='coord')return `<div class="nxt-game-stage"><h3>お題: 雨の日カフェコーデ</h3><p>テーマ一致・色・季節感・価格で採点。</p><div class="nxt-items">${['デニムジャケット','白シャツ','黒スラックス','赤スニーカー','レトロ帽子','革バッグ'].map(x=>`<button class="nxt-item" onclick="this.classList.toggle('sel')">${x}</button>`).join('')}</div><button class="nxt-play" onclick="NexcaTown.finishGame('coord')">決定</button></div>`;
    if(type==='order')return `<div class="nxt-game-stage"><h3>注文: 静かな席とホットコーヒー</h3><p>正しいメニューでコンボを伸ばそう。</p><div class="nxt-items">${['ホットコーヒー','アイスラテ','静かな席','写真映え席','チーズケーキ','抹茶ラテ'].map(x=>`<button class="nxt-item" onclick="this.classList.toggle('sel')">${x}</button>`).join('')}</div><button class="nxt-play" onclick="NexcaTown.finishGame('order')">提供する</button></div>`;
    if(type==='festival')return `<div class="nxt-game-stage"><h3>フェス準備パズル</h3><p>3x3に施設を配置して満足度・安全性・SNS映えを上げよう。</p><div class="nxt-fes-grid">${Array.from({length:9},(_,i)=>`<button class="nxt-cell" onclick="NexcaTown.placeCell(this)">${i===0?'入口':''}</button>`).join('')}</div><div class="nxt-items" style="margin-top:10px">${['ステージ','受付','屋台','休憩所','写真スポット','救護所'].map(x=>`<button class="nxt-item" onclick="NexcaTown.pickFacility('${x}')">${x}</button>`).join('')}</div><button class="nxt-play" onclick="NexcaTown.finishGame('festival')">会場を公開</button></div>`;
    ...
    return `<div class="nxt-game-stage"><h3>${type==='manager'?'課題: 予約率が低い':'お題: 高校生に刺さる投稿'}</h3><p>具体性・不安解消・行動導線で評価します。</p><div class="nxt-items">${opts.map((x,i)=>`<button class="nxt-choice" onclick="NexcaTown.selectChoice(this,${i})">${x}</button>`).join('')}</div><button class="nxt-play" onclick="NexcaTown.finishGame('${type}')">結果を見る</button></div>`;
  }
