(function(){
  ...
  .nx-video-frame{position:absolute;inset:0;width:100%;height:100%;border:0;background:#050507}.nx-thumb{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:radial-gradient(circle at 50% 35%,rgba(255,255,255,.13),transparent 48%),linear-gradient(135deg,#111827,#050507)}.nx-thumb-img{position:absolute;inset:0;background-size:cover;background-position:center;filter:saturate(1.08) contrast(1.04)}.nx-thumb-img::after{content:"";position:absolute;inset:0;background:linear-gradient(180deg,rgba(0,0,0,.05),rgba(0,0,0,.52))}.nx-play{position:relative;z-index:2;width:72px;height:72px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,.5);border:1px solid rgba(255,255,255,.2);font-size:28px;backdrop-filter:blur(10px)}
  ...
  .nx-near-btn{position:fixed;right:14px;bottom:88px;z-index:120;width:54px;height:54px;border:0;border-radius:18px;background:linear-gradient(135deg,#4d9fff,#00e5a0);color:#03120d;font-size:22px;font-weight:1000;box-shadow:0 12px 28px rgba(0,0,0,.35);touch-action:manipulation}.nx-near-panel{position:fixed;left:10px;right:10px;bottom:78px;z-index:210;background:rgba(10,10,15,.96);border:1px solid rgba(255,255,255,.12);border-radius:18px;padding:13px;box-shadow:0 20px 45px rgba(0,0,0,.4);display:none}.nx-near-panel.on{display:block}.nx-near-head{display:flex;align-items:center;gap:8px;margin-bottom:10px}.nx-near-title{font-size:13px;font-weight:1000}.nx-near-close{margin-left:auto;border:0;background:rgba(255,255,255,.08);color:white;border-radius:999px;width:30px;height:30px}.nx-near-list{display:flex;gap:8px;overflow-x:auto;scrollbar-width:none}.nx-near-card{flex:0 0 178px;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);border-radius:14px;padding:10px}.nx-near-card-t{font-size:12px;font-weight:1000;line-height:1.35}.nx-near-card-s{font-size:10px;color:var(--dim);line-height:1.5;margin-top:5px}.nx-town-quest{position:absolute;left:10px;top:10px;right:10px;display:grid;grid-template-columns:1fr auto;gap:8px;z-index:4}.nx-town-quest-main{background:rgba(0,0,0,.38);border:1px solid rgba(255,255,255,.12);border-radius:13px;padding:9px;backdrop-filter:blur(8px)}.nx-town-quest-t{font-size:10px;color:var(--yellow);font-weight:1000}.nx-town-quest-d{font-size:11px;color:white;line-height:1.45;margin-top:3px}.nx-town-quest-btn{border:0;border-radius:13px;background:var(--yellow);color:#1a1508;font-size:11px;font-weight:1000;padding:0 12px}
  ...
  function extendData(){
    ...
    if(typeof OC!=='undefined'&&!OC.ネクスケ)OC.ネクスケ={color:'#E63946',genre:'event',kuchi:'次、行ってみよ。',pers:'Nexcaそのものの好奇心を持つ案内役。',weak:'予定を詰め込みすぎる。',compat:'ネクスケ',cd:'勢いと観察眼の相性がいい。',catch:'まだ知らない広島を、今日の予定に変える。',mbti:{E:70,N:88,F:60,P:90},strengths:['発見力','案内力','巻き込み力'],growth:'休む時間も予定に入れて。',lvUp:[]};
  ...
  const curatedPlans=[
    {title:'本通り3択ミッション',desc:'最初の角で右・左・直進をじゃんけんで決める。最後に一番よかった看板を撮って今日の勝者を決める。',area:'広島市中区',mood:'ボケ',time:'午後',budget:500,indoor:false,outdoor:true,mission:'変な看板を1つ見つけて、誰が一番ネーミングセンスあるか決める',map:'https://maps.google.com/?q=本通り 広島'},
    {title:'袋町チル会議',desc:'袋町周辺で飲み物を買って、公園か近くのカフェで「次に行きたい場所」を1つずつ出す。',area:'広島市中区',mood:'まったり',time:'夕方',budget:1000,indoor:true,outdoor:true,mission:'今日の会話から次回の行き先を1つ決める',map:'https://maps.google.com/?q=袋町公園'},
    {title:'宇品港エモ回収',desc:'海を見ながら昔の写真を1枚だけ見返す。話しすぎず、最後に一言だけ感想を残す。',area:'広島市南区',mood:'エモい',time:'夕方',budget:300,indoor:false,outdoor:true,mission:'スマホをしまって3分だけ海を見る',map:'https://maps.google.com/?q=宇品港'},
    {title:'尾道迷子デート',desc:'細い路地を地図なしで歩き、猫の細道か坂道で一番好きな景色を見つける。明るい時間帯推奨。',area:'尾道市',mood:'ロマンチック',time:'昼',budget:3000,indoor:false,outdoor:true,mission:'お互いのベスト景色を1枚ずつ撮る',map:'https://maps.google.com/?q=尾道駅'},
    {title:'雨の日カフェ潜伏',desc:'天気が悪い日は屋外を捨てて、駅近か商店街のカフェで作戦会議。帰り道だけ少し寄り道する。',area:'広島市中区',mood:'まったり',time:'昼',budget:1000,indoor:true,outdoor:false,mission:'次に晴れた日に行く場所を1つ保存する',map:'https://maps.google.com/?q=広島市中区 カフェ'}
  ];
  ...
  function renderGroupResult(p){
    ...
    el.innerHTML=`<div class="gg-result-card"><div class="gg-rarity ${p.rarity}">${p.rarity==='legendary'?'激レア 1日プラン':p.rarity==='rare'?'レアプラン':'ノーマルプラン'}</div><div style="font-size:18px;font-weight:1000;line-height:1.35;margin-bottom:7px;">${safe(p.title)}</div><div style="font-size:12px;color:var(--txt2);line-height:1.85;">${safe(p.desc)}</div><div style="margin-top:10px;border:1px solid rgba(255,190,0,.24);background:rgba(255,190,0,.08);border-radius:13px;padding:10px;"><div style="font-size:10px;color:var(--yellow);font-weight:1000;margin-bottom:4px;">TODAY MISSION</div><div style="font-size:12px;line-height:1.65;">${safe(p.mission)}</div></div><div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:11px;">${p.route.map(x=>`<span style="font-size:10px;font-weight:900;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.1);border-radius:999px;padding:5px 8px;">${safe(x)}</span>`).join('')}</div><div style="font-size:11px;color:var(--dim);margin-top:10px;">${safe(p.scene)} · ${safe(p.count)} · ${safe(p.budgetLabel)} · ${safe(p.area)}</div><div class="gg-actions"><a href="${p.map}" target="_blank">地図</a><button onclick="addPt('今日これをやる',20,false);toast('今日のプランに決定！ 参加コードがある場所なら証明できます')">今日これをやる</button><button onclick="shareGroupPlan()">シェア</button></div></div>`;
  ...
  const AREA_COORDS=[['広島市中区',34.392,132.455],['広島市西区',34.398,132.414],['広島市南区',34.377,132.469],['広島市東区',34.41,132.48],['呉市',34.249,132.565],['福山市',34.485,133.362],['尾道市',34.409,133.205],['東広島市',34.426,132.744],['廿日市市',34.349,132.331]];
  function distKm(a,b,c,d){const R=6371,rad=x=>x*Math.PI/180,dx=rad(c-a),dy=rad(d-b),v=Math.sin(dx/2)**2+Math.cos(rad(a))*Math.cos(rad(c))*Math.sin(dy/2)**2;return R*2*Math.atan2(Math.sqrt(v),Math.sqrt(1-v));}
  function nearestArea(lat,lng){return AREA_COORDS.slice().sort((x,y)=>distKm(lat,lng,x[1],x[2])-distKm(lat,lng,y[1],y[2]))[0];}
  function eventDistance(ev,lat,lng){
    if(Number(ev.lat)&&Number(ev.lng))return distKm(lat,lng,Number(ev.lat),Number(ev.lng));
    const text=String(ev.loc||ev.addr||'');
    const hit=AREA_COORDS.find(a=>text.includes(a[0])||text.includes(a[0].replace('広島市','')));
    return hit?distKm(lat,lng,hit[1],hit[2]):999;
  }
  function installNearMe(){
    if($('#nx-near-btn'))return;
    const btn=document.createElement('button');btn.id='nx-near-btn';btn.className='nx-near-btn';btn.type='button';btn.textContent='📍';btn.onclick=openNearMe;document.body.appendChild(btn);
    const p=document.createElement('div');p.id='nx-near-panel';p.className='nx-near-panel';p.innerHTML=`<div class="nx-near-head"><div class="nx-near-title">現在地から探す</div><button class="nx-near-close" onclick="document.getElementById('nx-near-panel').classList.remove('on')">×</button></div><div class="nx-near-list" id="nx-near-list"></div>`;document.body.appendChild(p);
  }
  window.openNearMe=function(){
    const panel=$('#nx-near-panel'),list=$('#nx-near-list'); if(panel)panel.classList.add('on'); if(list)list.innerHTML='<div class="nx-near-card"><div class="nx-near-card-t">現在地を取得中</div><div class="nx-near-card-s">許可すると近い順に動画候補を出します</div></div>';
    if(!navigator.geolocation){renderNearFallback();return;}
  ...
  function overrideCore(){
    ...
    window.renderFlyer=function(){
      const evs=flyG==='all'?(EVS||[]).filter(e=>!e.isOfficial):(EVS||[]).filter(e=>!e.isOfficial&&(e.g===flyG||e.genre===flyG));
      const grid=$('#flgrid'); if(!grid)return;
      if(!evs.length){grid.innerHTML='<div class="empty">イベントがありません</div>';return;}
      grid.innerHTML=evs.map(ev=>{
        const thumb=ev.thumbnail_url?`<div class="nx-thumb-img" style="background-image:url('${safe(ev.thumbnail_url)}');"></div>`:`<div class="fct-em">${safe(ev.em||ev.ge||'🎉')}</div>`;
        return `<div class="fc" onclick="openVOV(this.dataset.id)" data-id="${safe(ev.id)}"><div class="fct" style="background:${safe(ev.bg)};">${thumb}<div class="fct-ovl"></div><div class="fct-play">▶</div></div><div class="fci"><div class="fct-t">${safe(ev.title)}</div></div></div>`;
      }).join('');
      const mp=$('#mplist'); if(mp)mp.innerHTML=evs.map(ev=>`<a class="mpitem" href="https://maps.google.com/?q=${encodeURIComponent(ev.addr||ev.loc||'')}" target="_blank">${safe(ev.em||ev.ge||'🎉')} ${safe(ev.title)}</a>`).join('');
    };
    window.openVOV=function(id){
      const ev=(EVS||[]).find(e=>e.id===id); if(!ev)return;
      const map=`https://maps.google.com/?q=${encodeURIComponent(ev.addr||ev.loc||'')}`;
      const vt=$('#vov-t'),vc=$('#vov-c'),vd=$('#vov-d'),v=$('#vov');
      if(vt)vt.textContent=ev.title;
      if(vc)vc.innerHTML=`<div style="position:absolute;inset:0;background:${safe(ev.bg)};">${videoHtml(ev)}</div>`;
      if(vd)vd.innerHTML=`<div style="font-size:14px;font-weight:900;margin-bottom:6px;">${safe(ev.title)}</div><div style="font-size:12px;color:var(--dim);line-height:1.6;margin-bottom:12px;">${safe(ev.loc||'場所未設定')} · ${safe(ev.price||'料金未設定')}</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;"><button class="cta cta-h" onclick="closeVOV();openDrw(this.dataset.id)" data-id="${safe(ev.id)}" style="width:100%;padding:10px;">詳細情報</button><a class="cta cta-m" href="${map}" target="_blank" onclick="trackLinkClick('${safe(ev.id)}','map')" style="width:100%;padding:10px;text-align:center;text-decoration:none;">地図</a></div>`;
  ...
  const NX_DIAG=[
    ['初めての場所に行く前、近いのは？',[['すぐ現地で感じたい',{e:2,a:2}],['写真や口コミを見て安心したい',{c:2}],['誰かと一緒なら行ける',{e:1,r:1}],['気分が整った日だけ行く',{r:2}]]],
    ['予定が急に空いたら？',[['外に出る理由を探す',{a:2,e:1}],['行きたい保存を見返す',{c:1}],['カフェでゆっくりする',{r:2}],['家で回復する',{r:1}]]],
    ['動画で一番見たい情報は？',[['雰囲気と混み具合',{r:1}],['料金と場所',{c:2}],['友達と盛り上がる場面',{e:2}],['変わった体験の瞬間',{a:2}]]],
    ['友達を誘うときの言葉は？',[['絶対おもろいから行こう',{e:2}],['これ好きそうじゃない？',{r:2}],['時間と料金これならどう？',{c:2}],['ノリで決めよ',{a:2}]]],
    ['地域イベントに期待するものは？',[['新しい出会い',{e:2}],['自分の世界が広がる感じ',{a:2}],['落ち着ける居場所',{r:2}],['学びや制作の手応え',{c:2}]]],
    ['参加費の感覚は？',[['無料から試したい',{c:1}],['500円くらいなら気軽',{r:1}],['価値があれば払う',{a:1}],['特別感には課金したい',{e:1,a:1}]]],
    ['一人参加のしやすさは？',[['全然いける',{a:2}],['静かな場所なら',{r:2}],['目的が明確なら',{c:2}],['誰かと一緒がいい',{e:1}]]],
    ['広島で増えてほしい場所は？',[['若者が集まる場',{e:2}],['映えるけど落ち着く店',{r:2}],['挑戦できる体験',{a:2}],['学べる制作拠点',{c:2}]]],
    ['主催者情報で信用するものは？',[['写真・動画のリアルさ',{r:1}],['公式サイトや料金表',{c:2}],['口コミや参加者数',{e:1}],['運営承認マーク',{c:1}]]],
    ['好きな寄り道は？',[['古着屋',{c:1,r:1}],['路地裏カフェ',{r:2}],['川沿い散歩',{r:1}],['知らない駅で降りる',{a:2}]]],
    ['失敗したくない予定は？',[['デート',{r:2}],['友達との集合',{e:2}],['初参加イベント',{c:2}],['旅行先の1日',{a:2}]]],
    ['おすすめの出方は？',[['今近い場所',{a:1}],['年齢と地域に合うもの',{c:2}],['友達と盛り上がるもの',{e:2}],['静かに刺さるもの',{r:2}]]],
    ['参加後に残したいものは？',[['写真',{r:1}],['ポイントと称号',{a:1}],['キャラ育成',{e:1}],['自分の記録',{c:1}]]],
    ['ネクスカに一番欲しいものは？',[['行く理由',{a:2}],['安心感',{c:2}],['ワクワク',{e:2}],['自分らしい居場所',{r:2}]]],
    ['最後に、今のあなたは？',[['退屈を変えたい',{a:2}],['誰かと笑いたい',{e:2}],['自分を知りたい',{c:2}],['少し癒されたい',{r:2}]]]
  ];
  const DIAG_CHARS={イロドリ:['熱量拡散型','場の温度を一段上げる人。外向性と新奇探索が高く、体験の入り口で迷う人の背中を押せる。','続きの物語：イロドリは街の小さな火花を集め、退屈な午後を祭りに変える。'],ツギハ:['審美編集型','雰囲気、意味、手触りをつなぐ人。好き嫌いの解像度が高く、場所の良さを言語化できる。','続きの物語：ツギハは古い布の記憶から、その人だけの今日の服を編む。'],コモレビ:['安心共感型','安全基地を作れる人。共感性と回復志向が強く、誰かがほっとできる選択を見つける。','続きの物語：コモレビのいる席では、急がなくていい時間だけが流れる。'],シルベ:['探究実装型','仕組みを見抜き、試して形にする人。学習欲と計画性が高く、体験を自分のスキルに変える。','続きの物語：シルベは街の裏側にある装置を直し、明日の扉を開ける。'],ヨドミ:['没頭創造型','深く潜って独自の世界を作る人。内省と創造性が高く、流行の奥にある美しさを見つける。','続きの物語：ヨドミの机から生まれた小さな案は、やがて街の合言葉になる。'],ネクスケ:['観察余白型','静かに見て、本質を拾う人。刺激より余白を大事にし、自分のペースで深く楽しめる。','続きの物語：ネクスケは風の向きだけで、その日いちばん優しい道を知っている。'],ネクスケ:['発見案内型','好奇心と安全感のバランスが良い案内役。人、場所、予定をつなぐのが得意。','続きの物語：ネクスケは地図にない印を見つけ、今日を少しだけ特別にする。'],ネクスケ:['静かな突破型','少人数や裏側で力を発揮する人。派手さより確かな行動で、体験を深く記憶に残す。','続きの物語：ネクスケは夜道の危ない角を避け、光のある場所へ仲間を導く。']};
  let nxDQ=[],nxQI=0,nxScore={},nxAns={};
  window.startDiag=function(){nxDQ=NX_DIAG;nxQI=0;nxScore={e:0,a:0,c:0,r:0};nxAns={};$('#diag-start').style.display='none';$('#diag-result').style.display='none';$('#diag-q').style.display='flex';renderNxQ();};
  ...
  const OFFICIAL_CHARS={
    'イロドリ':{genre:'event',color:'#ff4d4d',icon:'🔥',evo:['火花のイロドリ','熱風のイロドリ','爆祭のイロドリ'],line:['よし、今日を動かすぞ。','その一歩、ちゃんと熱になってる。','街ごと上げていこう。']},
    'ツギハ':{genre:'vintage',color:'#ffd54a',icon:'🧵',evo:['目利きのツギハ','記憶編みのツギハ','時装のツギハ'],line:['その服、物語を持ってる。','好きはちゃんと積み重なる。','君の審美眼、進化してる。']},
    'コモレビ':{genre:'cafe',color:'#6ee7a8',icon:'☕',evo:['ひだまりのコモレビ','深煎りのコモレビ','憩界のコモレビ'],line:['まあ座って、息をしよう。','休むのも前に進むこと。','ここが君の充電スポット。']},
    'シルベ':{genre:'event',color:'#7c8cff',icon:'⚙️',evo:['試作のシルベ','発明のシルベ','未来図のシルベ'],line:['失敗はデータ。次いこ。','発見、ちゃんと記録した？','君の工夫が街を変える。']},
    'ヨドミ':{genre:'event',color:'#c084fc',icon:'🫧',evo:['糸口のヨドミ','結び目のヨドミ','縁環のヨドミ'],line:['つながりは小さく始まる。','話してみる価値、あるかも。','その縁、育ってきたね。']},
    'ネクスケ':{genre:'cafe',color:'#67d8ff',icon:'🪽',evo:['そよぎのネクスケ','雲間のネクスケ','風詠みのネクスケ'],line:['急がなくていいよ。','見えてるもの、言葉にしてみて。','軽やかに、でも確かに。']},
    'ネクスケ':{genre:'event',color:'#e63946',icon:'🧭',evo:['案内人ネクスケ','開拓者ネクスケ','Nexcaの羅針盤'],line:['次の広島、見つけに行こ。','今日の選択が地図になる。','君がNexcaを進めてる。']},
    'ネクスケ':{genre:'event',color:'#9aa0ff',icon:'🌙',evo:['月影のネクスケ','静守のネクスケ','夜導のネクスケ'],line:['静かな参加も、ちゃんと力だ。','見えないところで街は育つ。','君の一歩、誰かを支えてる。']}
  };
  function charForEvent(ev){if(ev.charKey&&OFFICIAL_CHARS[ev.charKey])return ev.charKey;if(ev.g==='vintage'||ev.genre==='vintage')return 'ツギハ';if(ev.g==='cafe'||ev.genre==='cafe')return 'コモレビ';const pool=['イロドリ','ネクスケ','ヨドミ','シルベ','ネクスケ'];return pool[Math.abs(String(ev.id||ev.title).split('').reduce((a,c)=>a+c.charCodeAt(0),0))%pool.length];}
  function charSvg(name,size=72){
    const c=OFFICIAL_CHARS[name]||OFFICIAL_CHARS.ネクスケ, icon=c.icon, hue=c.color;
    const parts={
      'イロドリ':'<path d="M42 22 C48 4 58 15 60 3 C66 17 82 9 76 29" fill="#ffbe00" opacity=".9"/><path d="M35 88 C44 108 78 108 86 88" fill="none" stroke="#ff8c00" stroke-width="8" stroke-linecap="round"/>',
      'ツギハ':'<path d="M24 42 C12 24 35 18 43 36" fill="#fff4bf"/><path d="M96 42 C108 24 85 18 77 36" fill="#fff4bf"/><path d="M31 86 C44 76 75 76 89 86" fill="none" stroke="#8b5e34" stroke-width="4" stroke-dasharray="4 5"/>',
      'コモレビ':'<path d="M28 34 C18 18 39 18 45 35" fill="#d7ffe7"/><path d="M75 27 C93 17 101 35 84 43" fill="#d7ffe7"/><path d="M37 84 C45 93 76 93 84 84" fill="none" stroke="#5a3b20" stroke-width="5" stroke-linecap="round"/>',
      'シルベ':'<path d="M24 55 h-12 M108 55 h-12 M60 18 v-12" stroke="#dce7ff" stroke-width="6" stroke-linecap="round"/><circle cx="60" cy="60" r="48" fill="none" stroke="#dce7ff" stroke-width="3" stroke-dasharray="7 8"/>',
      'ヨドミ':'<path d="M22 72 C34 46 47 46 60 72 C73 98 86 98 98 72" fill="none" stroke="#f0d7ff" stroke-width="7" stroke-linecap="round"/><circle cx="30" cy="34" r="8" fill="#f0d7ff" opacity=".9"/><circle cx="91" cy="33" r="6" fill="#f0d7ff" opacity=".75"/>',
      'ネクスケ':'<path d="M20 58 C24 30 48 37 52 57 C41 62 31 64 20 58Z" fill="#e2f8ff"/><path d="M100 58 C96 30 72 37 68 57 C79 62 89 64 100 58Z" fill="#e2f8ff"/><path d="M41 91 C54 99 68 99 81 91" fill="none" stroke="#dff9ff" stroke-width="5" stroke-linecap="round"/>',
      ...
      'ネクスケ':'<path d="M84 26 C62 28 50 46 54 66 C58 84 75 92 92 86 C82 102 44 96 34 70 C25 45 48 20 84 26Z" fill="#e9eaff" opacity=".9"/><circle cx="36" cy="30" r="4" fill="#fff" opacity=".8"/>'
    };
  ...
  function enhanceGacha(){
    ...
    GACHA.Common=[
      ...
      {text:'今日の気分に合うカフェを1つ保存して、休憩予定を作る',ev:'休憩の予定化は外出の心理的負担を下げる',type:'家',pts:60,char:'コモレビ'},
      {text:'古着ジャンルを見て、今の自分にない色を1つ決める',ev:'選択肢の拡張は自己表現の幅を広げる',type:'家',pts:60,char:'ツギハ'}
    ];
    GACHA.Uncommon=[
      {text:'徒歩10分以内で入ったことのない店の前まで行く',ev:'近距離の新奇探索は継続しやすい',type:'外',pts:90,char:'イロドリ'},
      {text:'友達に「ここ一緒に行かん？」を1件送る',ev:'共有は参加ハードルを下げ、行動を現実に近づける',type:'家',pts:95,char:'ヨドミ'},
      {text:'今日見つけた広島の“いい違和感”を写真で残す',ev:'観察の習慣は街への愛着を高める',type:'外',pts:100,char:'ネクスケ'}
    ];
    GACHA.Rare=[
      {text:'Nexca掲載スポットを開いて、地図・料金・参加方法まで確認する',ev:'具体的な移動イメージは実参加率を高める',type:'外',pts:150,char:'シルベ'},
      {text:'カフェ→古着→川沿いを90分の小さな旅として回る',ev:'連続した体験は記憶に残りやすい',type:'外',pts:170,char:'ツギハ'}
    ];
    GACHA.Epic=[
      {text:'今日の広島を「朝・昼・夕方」の3枚で記録して、1本の物語にする',ev:'体験を物語化すると満足度と継続意欲が上がる',type:'外',pts:240,char:'ネクスケ'},
      {text:'Nexcaで見つけた場所に実際に行き、参加コード獲得まで狙う',ev:'完了体験は自己効力感を強く伸ばす',type:'外',pts:280,char:'ネクスケ'}
    ];
    GACHA.Legendary=[
      {text:'午前は街歩き、昼は地元ごはん、午後はイベント、夜は駅周辺で締める「広島1日クエスト」を達成する',ev:'長時間の新奇体験は街への愛着と自己効力感を大きく刺激する',type:'外',pts:420,char:'イロドリ'},
      {text:'誰かを1人誘って、Nexca掲載体験を本当に1つやり切る',ev:'共同達成は記憶の強度を高め、次の参加につながる',type:'外',pts:460,char:'ヨドミ'}
    ];
  }
  ...
  function drawPremiumMission(){
    ...
    mission.rarity=rarity; mission.char=mission.char||(['イロドリ','ツギハ','コモレビ','シルベ','ヨドミ','ネクスケ','ネクスケ','ネクスケ'][Math.floor(Math.random()*8)]);
    return mission;
  }
