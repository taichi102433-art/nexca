(function(){
  ...
  .gg-panel{display:flex;flex-direction:column;gap:12px}.gg-field{background:rgba(255,255,255,.035);border:1px solid rgba(255,255,255,.075);border-radius:12px;padding:11px}.gg-label{font-size:10px;font-weight:900;color:var(--dim);letter-spacing:1px;margin-bottom:8px}.gg-chips{display:flex;gap:7px;flex-wrap:wrap}.gg-chip{padding:8px 10px;border-radius:999px;background:rgba(255,255,255,.05);border:1.5px solid rgba(255,255,255,.08);font-size:11px;font-weight:800;color:var(--txt);cursor:pointer}.gg-chip.on{background:rgba(230,57,70,.18);border-color:var(--red);box-shadow:0 0 18px rgba(230,57,70,.18)}.gg-btn.destiny{width:100%;padding:15px;background:linear-gradient(135deg,#E63946,#ffbe00);box-shadow:0 0 24px rgba(230,57,70,.35)}.gg-spin{padding:18px;text-align:center;font-family:var(--font-en);font-size:20px;letter-spacing:2px;color:var(--yellow);animation:pulse .35s infinite alternate}.gg-result-card{border-radius:16px;padding:16px;background:linear-gradient(160deg,rgba(255,255,255,.09),rgba(255,255,255,.03));border:1px solid rgba(255,255,255,.12)}.gg-rarity{display:inline-flex;padding:3px 10px;border-radius:999px;font-size:10px;font-weight:900;margin-bottom:8px}.gg-rarity.normal{background:rgba(255,255,255,.08);color:var(--txt2)}.gg-rarity.rare{background:rgba(77,159,255,.16);color:var(--blue)}.gg-rarity.legendary{background:rgba(255,190,0,.2);color:var(--yellow);box-shadow:0 0 22px rgba(255,190,0,.2)}.gg-actions{display:flex;gap:8px;margin-top:12px}.gg-actions button,.gg-actions a{flex:1;text-align:center;text-decoration:none;padding:10px;border-radius:10px;border:0;font-size:12px;font-weight:900;color:white;background:rgba(255,255,255,.08);font-family:var(--font-jp)}
  ...
  .gacha-legendary-burst{animation:nxBurst 1.4s ease both}@keyframes nxBurst{35%{box-shadow:0 0 0 999px rgba(255,190,0,.16)}}@media (orientation:landscape){#nav{height:50px}.screen{bottom:50px}.sbot{bottom:8px}.srt{bottom:62px}.stitle{font-size:17px}.ob-logo{font-size:40px}}`;
  ...
  function hydrateGroupGacha(){
    ...
    old.innerHTML='<div style="font-family:var(--font-en);font-size:15px;letter-spacing:1px;margin-bottom:14px;">GROUP GACHA</div><div id="gg-v2-panel" class="gg-panel"></div><button class="gg-btn destiny" onclick="drawGroupGacha()">運命を回す</button><div class="gg-result" id="gg-result"></div>';
  ...
  window.drawGroupGacha=function(){
    ...
    el.style.display='block'; el.innerHTML='<div class="gg-spin">ROULETTE...</div>';
    setTimeout(()=>el.innerHTML='<div class="gg-spin">DRUM ROLL...</div>',3000);
    setTimeout(()=>{
      const r=Math.random()*100, rarity=r<5?'legendary':r<30?'rare':'normal', pool=rarity==='legendary'?dayPlans:plans;
      ...
      const p=evHit&&rarity!=='legendary'?{title:evHit.title,desc:'Nexca掲載イベントが条件に合ったので優先表示。'+evHit.desc,area:evHit.loc,mood:moodVals[0],time:evHit.ts||times[0],budget:evHit.price,map:`https://maps.google.com/?q=${encodeURIComponent(evHit.addr||evHit.loc)}`}:(cand[Math.floor(Math.random()*cand.length)]||pool[Math.floor(Math.random()*pool.length)]);
      window.lastGroupPlan=p;
      el.innerHTML=`<div class="gg-result-card"><div class="gg-rarity ${rarity}">${rarity==='legendary'?'激レア 1日プラン':rarity==='rare'?'レア':'ノーマル'}</div><div style="font-size:17px;font-weight:900;margin-bottom:6px;">${safe(p.title)}</div><div style="font-size:12px;color:var(--txt2);line-height:1.8;">${safe(p.desc)}</div><div style="font-size:11px;color:var(--dim);margin-top:10px;">${safe(scene)} · ${safe(count)} · ${safe(budget)} · ${safe(p.area)}</div><div class="gg-actions"><a href="${p.map}" target="_blank">地図</a><button onclick="addPt('今日これをやる',10,false);toast('今日のプランに決定！ +10pt')">今日これをやる</button><button onclick="shareGroupPlan()">シェア</button></div></div>`;
